...
class SvgChart:
    ...
    def text(
        ...
        y,
        content,
        size=26,  # Aumentato del 20% (da 22 a 26)
        weight="normal",
        anchor="start",
        fill="#111",
        rotate=None,
...
def draw_legend(chart: SvgChart, items: Iterable[tuple[str, str]], x: float, y: float) -> None:
    ...
    for label, color in items:
        # Quadratino del colore ancora più grande per bilanciare il font
        chart.rect(x + offset, y - 18, 24, 24, fill=color, stroke="#222", stroke_width=0.6)
        chart.text(x + offset + 36, y + 2, label, size=24)  # Font aumentato da 20 a 24
        offset += 36 + len(label) * 14 + 24  # Offset orizzontale maggiorato
...
def draw_pairwise_delta_connectors(
    ...
    for idx in range(len(values) - 1):
        ...
        label_y = min(y1, y2) - 14  # Allontanato il testo dalla linea
        label_y = max(floor_y, label_y)
        chart.text(
            (x1 + x2) / 2,
            label_y,
            fmt_delta_percent(values[idx], values[idx + 1]),
            size=19,  # Font aumentato da 16 a 19
            weight="bold",
            anchor="middle",
            fill=stroke,
        )
...
def bar_chart(
    ...
    chart = SvgChart()
    left, right = 140, 60  # 'left' aumentato da 120 a 140
    top, bottom = (90 if show_delta_connectors else 50), 150  # 'bottom' aumentato da 130 a 150, 'top' riadattato
    ...
    chart.text(42, top + plot_h / 2, y_axis_label, size=24, rotate=-90, anchor="middle", fill="#444")  # Font da 20 a 24, offset x da 32 a 42

    tick = 0.0
    while tick <= ymax + 1e-9:
        y = top + plot_h - (tick / ymax) * plot_h
        chart.line(left, y, left + plot_w, y, stroke="#D9D9D9", stroke_width=1)
        chart.text(left - 15, y + 8, axis_formatter(tick), size=20, anchor="end", fill="#555")  # Font da 17 a 20, offset x e y aumentati
        tick += step
    ...
    for idx, (label, value, color) in enumerate(zip(labels, values, colors)):
        ...
        chart.text(cx, y - 15, value_formatter(value), size=22, anchor="middle", fill="#222")  # Font da 18 a 22, offset y a -15
        chart.text(cx, top + plot_h + 45, label, size=24, anchor="middle")  # Font da 20 a 24, offset y a 45
...
def grouped_bar_chart(
    ...
    chart = SvgChart()
    draw_legend(chart, list(zip(series_names, series_colors)), 160, 44)

    left, right = 140, 60  # 'left' da 120 a 140
    top, bottom = 100, 150  # 'bottom' da 130 a 150
    plot_w = chart.width - left - right
    plot_h = chart.height - top - bottom
    ...
    chart.line(left, top + plot_h, left + plot_w, top + plot_h, stroke="#222", stroke_width=1.5)
    chart.line(left, top, left, top + plot_h, stroke="#222", stroke_width=1.5)
    chart.text(42, top + plot_h / 2, y_axis_label, size=24, rotate=-90, anchor="middle", fill="#444")  # Font da 20 a 24, offset x 42

    tick = 0.0
    while tick <= ymax + 1e-9:
        y = top + plot_h - (tick / ymax) * plot_h
        chart.line(left, y, left + plot_w, y, stroke="#D9D9D9", stroke_width=1)
        chart.text(left - 15, y + 8, axis_formatter(tick), size=20, anchor="end", fill="#555")  # Font da 17 a 20, offset y+8
        tick += step
    ...
    for category_idx, category in enumerate(categories):
        ...
        for series_idx, series in enumerate(series_names):
            ...
            chart.text(x + bar_w / 2, y - 12, value_formatter(value), size=17, anchor="middle", fill="#222")  # Font da 14 a 17, offset -12
        chart.text(left + group_w * (category_idx + 0.5), top + plot_h + 45, category, size=24, anchor="middle")  # Font da 20 a 24, offset 45
...
def percent_stacked_chart(
    ...
    left, right = 140, 60  # 'left' da 120 a 140
    top, bottom = (140 if connector_values is not None else 100), 150  # 'bottom' da 130 a 150
    plot_w = chart.width - left - right
    plot_h = chart.height - top - bottom

    chart.line(left, top + plot_h, left + plot_w, top + plot_h, stroke="#222", stroke_width=1.5)
    chart.line(left, top, left, top + plot_h, stroke="#222", stroke_width=1.5)
    chart.text(42, top + plot_h / 2, "Share of end-to-end latency [%]", size=24, rotate=-90, anchor="middle", fill="#444")  # Font da 20 a 24

    for pct in range(0, 101, 20):
        y = top + plot_h - (pct / 100.0) * plot_h
        chart.line(left, y, left + plot_w, y, stroke="#D9D9D9", stroke_width=1)
        chart.text(left - 15, y + 8, f"{pct}", size=20, anchor="end", fill="#555")  # Font da 17 a 20, offset y+8
    ...
    for idx, category in enumerate(categories):
        ...
        chart.text(cx, top + plot_h + 45, category, size=24, anchor="middle")  # Font da 20 a 24, offset 45
